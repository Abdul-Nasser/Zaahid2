...
namespace {
  ...
  const Score BishopPawns         = S( 6,  7);
  const Score RookOnPawn          = S( 8, 24);
  const Score TrappedRook         = S(92,  0);
  const Score SafeCheck           = S(20, 20);
  const Score OtherCheck          = S(10,  0);
  const Score ThreatByHangingPawn = S(53, 24);
  const Score LooseEnemies        = S( 0, 25);
  const Score PinOnQueen          = S(20,  0);
  const Score Hanging             = S(78,  6);
  const Score ThreatByPawnPush    = S(49, 15);
  const Score Unstoppable         = S( 0, 39);
  ...
  Score evaluate_pieces(const Position& pos, EvalInfo& ei, const Bitboard* mobilityArea) {
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            if (Pt == BISHOP)
                score -= BishopPawns * ei.pi->bishop_penalty(Us, s);
  ...
  Score evaluate_king(const Position& pos, const EvalInfo& ei) {
    ...
    if (ei.kingAttackersCount[Them])
    {
        ...
        attackUnits =  std::min(995, ei.kingAttackersCount[Them] * ei.kingAttackersWeight[Them])
                     + 215 * ei.kingAdjacentZoneAttacksCount[Them]
                     + 510* popcount(undefended)
                     + 130 * popcount(b)
                     + 120 * (popcount(b) + !!pinned)

                     - !pos.count<QUEEN>(Them) * 700;
        ...
        if (ei.me->game_phase() < PHASE_MIDGAME)
        {
            if (pos.opposite_bishops())
            {
                ...
                // a bit drawish, but not as drawish as with only the two bishops.
                    ...
                    sf = pos.count<ALL_PIECES>(strongSide) - pos.count<PAWN>(strongSide) <= 3 ? ScaleFactor(40) : ScaleFactor(46);
            }
...
Value Eval::evaluate(const Position& pos) {
  ...
  Value v =  mg_value(score) * int(ei.me->game_phase()) * sf / SCALE_FACTOR_NORMAL
